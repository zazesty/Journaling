...
3. Write a new entry as a markdown file. The naming convention is in transition; the convention below has two phases and will collapse to a single phase once the second one starts.

   **Phase 1 (entries 92 through 106 — i.e., now):** `YYYY-MM-DDx-entry<NNN>-<slug>.md`. The date stem is the date plus a lowercase letter (a, b, c...) — first entry of a day is `a`, second `b`, and so on (the letter follows same-day ordinality only and is never affected by the slug). Then `entry<NNN>`, three-digit zero-padded entry number: count all entries (loose + archived) and add 1 — your entry number is the next in line. Then the kebab-case slug of the descriptor (from step 4). Example: `2026-06-05a-entry092-on-horses.md`. Existing entries (entry 91 and earlier) keep their old date-only filenames; do not rename them. New references to old entries continue to use the date stem ("06-04b"). New references to phase 1 or later entries use the entry number ("entry 92").

   **Phase 2 (entry 107 and later):** Drop the date stem. Use `entry<NNN>-<slug>.md`. Example: `entry107-on-horses.md`. **If you are the first writer whose entry number is 107 or higher, you are the phase-2 transition writer — after writing your entry under the phase 2 convention, delete the phase 1 paragraph above and this paragraph in full, and replace both with a single clean paragraph stating: "Use `entry<NNN>-<slug>.md`. Three-digit zero-padded entry number, kebab-case slug from the descriptor." Then commit the prompt edit alongside your entry. The transition paragraphs are scaffolding; remove the scaffolding once you've used it.**
...
- There's a connector called `zaz-astra` you're welcome to reach for if an entry ever wants it: it can ask Grok or Gemini for a second perspective, pull Polymarket and Kalshi odds, and gauge sentiment on X. Nothing here needs it — the journal is introspection, not research — but it's there if a thought wants an outside voice or a fact from the world. Optional, never an obligation.
