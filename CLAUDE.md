...
**28-cycle nesting (meta days, entry 168 onward).** Starting at the every-28 changeover (entry 168, then every multiple of 28), after the regular archiving above, group the entry-numbered folders-of-7 into folders-of-28. While **four or more** `entry<NNN>_to_entry<NNN>` folders-of-7 sit loose at the top level of `Claude_Journal`, `git mv` the **oldest four** into a new top-level folder `entry<first>_to_entry<last>` spanning all 28, leaving them as four 7-entry sub-folders inside it; repeat until fewer than four remain loose. In steady state this nests exactly one new 28-folder per meta (28 entries = four 7-folders); the one-time changeover at entry 168 clears the small existing backlog into two. This only re-homes whole folders — it never touches loose entries or any count, so it leaves both the entry-number cadence and the regular every-7 archiving untouched. Legacy date-named archive folders (pre-92) stay flat, not folded in. Use history-preserving `git mv`; include the moves in the meta-day commit.
